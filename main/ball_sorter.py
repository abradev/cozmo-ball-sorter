...
# jergreenlower = (16, 82, 156)
# jergreenupper = (38, 129, 255)
...
def alignRobot(find_ball):
	...
	for degree in range(0, 360, rotate):
        ...
        if find_ball:
            contour_size = locateObject(frame, np.array(greenLower), np.array(greenUpper))
        else:
            contour_size = locateObject(frame, np.array(orangeLower), np.array(orangeUpper))
...
def locateObject(frame, low, high):
	...
	presort = imutils.grab_contours(contours)
