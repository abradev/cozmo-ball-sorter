...
class GUI(tk.Tk):
    ...
    def show_frame(self, page_name):
        ...
        if page_name != "FindingBallsPage":
            config.moveCozmo = False
        else:
            config.moveCozmo = True
...
class CozmoReturnImage:
    ...
    def cozmoGetImage(self,robot: cozmo.robot.Robot,setColorPage=False):
        ...
        resizedImage = imutils.resize(npImage, width=config.frameWidth)
        setColorImage = imutils.resize(npImage, width=config.frameWidth)
        #drawn on here 
        greenDrawBool, g_x, g_y, g_w, g_h, g_cx, g_cy = main.detectGreenBall(resizedImage)
        orangeDrawBool, o_x, o_y, o_w, o_h, o_cx, o_cy = main.detectOrangeTray(resizedImage)
        drawingImage = main.drawRectangles(resizedImage,greenDrawBool, g_x, g_y, g_w, g_h, g_cx, g_cy,orangeDrawBool, o_x, o_y, o_w, o_h, o_cx, o_cy)
        if setColorPage:
            self.final_im = ImageTk.PhotoImage(Image.fromarray(setColorImage))
        else:
            self.final_im = ImageTk.PhotoImage(Image.fromarray(drawingImage))
...
class SetColorsPage(tk.Frame, CozmoReturnImage):
    ...
    def __init__(self, parent, controller):
        ...
        self.canvas = tk.Canvas(self.frame_1_0_video_feed, width = config.frameWidth, height = config.frameHeight, bg="red")
    ...
    def displayFeed(self,robot: cozmo.robot.Robot):
        while True:
            self.canvas.create_image(300,225,anchor="center",image=self.cozmoGetImage(robot,True))
...
class FindingBallsPage(tk.Frame,CozmoReturnImage):
    ...
    def __init__(self, parent, controller):
        ...
        #colorLogLabelt = tk.Label(self.frame_1_0_video_feed, text="Color Log Log Log Log Log Log Log Log Log Log Log Log  Log Log Log Log Log  Log Log Log Log Log", pady=3, bg="red",wraplength=750, font=("Fixedsys", 50),justify="center").pack()
        self.canvas.pack()
        self.frame_1_0_video_feed.grid(row=0,column=0, rowspan=2,columnspan=5)

        ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        instruction_frame = tk.Frame(self)
        instruction_frame.grid(row=2, column=1, rowspan=1, columnspan=4)
...
def endChecker(robot: cozmo.robot.Robot):
    while True:
        time.sleep(5)
        print(main.repeatAttempts)
        if main.repeatAttempts >= 3: #if the robot tries to pick up a ball that is already in the goal 3 times, exit out of the program
            ...
            break
    gui.show_frame("ProgramTerminatedPage")
...
def cozmo_program(robot: cozmo.robot.Robot):
    ...
    moveCozmoThread = threading.Thread(target=main.cozmoFindGreenBall, args=[robot])
    moveCozmoThread.start()
    endCheckerThread = threading.Thread(target=endChecker,args=[robot])
