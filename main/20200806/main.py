...
ballsToPick = 0 #total number of balls in the field to pick, set by a input by the user. Used to determine when to end the program
pickedBalls = 0 #counter for the number of balls the Cozmo has picked

tolerance = 50 #tolerance for the amount of pixels the object can be off from the center of the camera
...
def cozmoFindGreenBall(robot: cozmo.robot.Robot):
    ...
    greenCentered = False #boolean variable for signaling when the green ball is in the center of the camera
    
    while pickingBall == 0 and findingGoal == 0 and droppingBall == 0 and pickedBalls < ballsToPick:
        ...
        if greenRect_cx != None:
            ...
            if greenCentered: #when the robot is greenCentered on a ball, start moving towards it
                ...
                if greenRect_w < greenRectWidthLimit:
                    robot.drive_wheel_motors(20,20) #drive forward if rectangle is too small

                elif greenRect_w > greenRectWidthLimit + 50:
                    robot.drive_wheel_motors(-20,-20) #back up if rectangle is too big
...
def find_goal(robot: cozmo.robot.Robot):
    ...
    orangeCentered = False #boolean variable for signaling when the orange tray is in the center of the camera
        ...
        if orangeRect_cx != None:
            ...
            if orangeCentered: #when the robot is orangeCentered on a ball, start moving towards it
                ...
                if orangeRect_w < orangeRectWidthLimit:
                    robot.drive_wheel_motors(20,20) #move forward if rectangle is too small
...
def drop_ball(robot: cozmo.robot.Robot):
    ...
    if droppingBall == 1 and findingGoal == 0:
        ...
        pickingBall = 0
        findingGoal = 0
        pickedBalls += 1 #increase counter by 1
...
def cozmo_program(robot: cozmo.robot.Robot):
    ...
    ballsToPick = int(input("How many balls are on the field for Cozmo to pick? >>> ")) #integer input to set variable
    ...
    while True: #loop images
        ...
        time.sleep(.003)
        if pickedBalls == ballsToPick: #if the cozmo has picked the number of balls specified, break out of the loop
            break
