...
#import video_feed as videoFeed
import cozmo
...
def detectGreenBall(image):
    ...
    #frame = imutils.resize(image, width=config.frameWidth) #resize the frame
...
def drawRectangles(image, greenDrawBool, g_x, g_y, g_w, g_h, g_cx, g_cy, orangeDrawBool, o_x, o_y, o_w, o_h, o_cx, o_cy):
    #frame = cv2.cvtColor(np.array(image), cv2.COLOR_RGB2BGR)
    #frame = imutils.resize(frame, width=config.frameWidth)   
    frame = image
...
def cozmoFindGreenBall(robot: cozmo.robot.Robot):
    ...
    while pickingBall == 0 and findingGoal == 0 and droppingBall == 0 and config.moveCozmo: #making sure that other frames are not up while the cozmo is moving
        print("hello")
...
def find_goal(robot: cozmo.robot.Robot):
    ...
    while pickingBall == 2 and findingGoal == 1 and droppingBall == 0 and config.moveCozmo: # if ball has been picked update
