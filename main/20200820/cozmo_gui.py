...
class GUI(tk.Tk):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.frames = {}
        for F in (HomePage, TutorialPage, AboutPage, SetColorsPage): #Namse of all the pages
...
class SetColorsPage(tk.Frame):
    ...
    def __init__(self, parent, controller):
        ...
        addBallColorBtn = tk.Button(frame_0_0, text="Add Ball Color", bg="white", font=controller.textFont, command=partial(enterColorIntoList,"ballColorDict",frame_0_0,str(ballColorEntries),str(ballColorEntries))).pack()
        deleteBallColorBtn = tk.Button(frame_0_0, text="Delete Ball Color", bg="white",font=controller.textFont, command=partial(deleteColorFromList,"ballColorDict",frame_0_0,1)).pack()
        ...
        addTrayColorBtn = tk.Button(frame_0_1, text="Add Tray Color", bg="white", font=controller.textFont, command=partial(enterColorIntoList,"trayColorDict",frame_0_1,"1,1,1","2,2,2")).pack()
        frame_0_1.grid(row=1, column=0)
        ''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
        frame_0_2 = tk.Frame(self)
        frame_0_2.configure(bg="white")

        finishButton = tk.Button(frame_0_2, text="finish", pady=3, bg="white", font=controller.buttonFont, height=controller.buttonHeight, width=int(controller.buttonWidth/3), command=lambda: controller.show_frame("HomePage"))
        finishButton.pack()
