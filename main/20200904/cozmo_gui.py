...
import cozmo_get_image as CozmoGetImage
...
def cozmoProcessImage(image, robot: cozmo.robot.Robot):
    drawingImage = None
    count = 0
    while True:
        greenDrawBool, g_x, g_y, g_w, g_h, g_cx, g_cy = main.detectGreenBall(image)
        orangeDrawBool, o_x, o_y, o_w, o_h, o_cx, o_cy = main.detectOrangeTray(image)
        drawingImage = main.drawRectangles(image,greenDrawBool, g_x, g_y, g_w, g_h, g_cx, g_cy,orangeDrawBool, o_x, o_y, o_w, o_h, o_cx, o_cy)
        time.sleep(CONFIG.refreshTime)
        return drawingImage
        print(count)
        count += 1
...
class FindingBallsPage(tk.Frame,CozmoReturnImage):
    ...
    def displayFeed(self,robot: cozmo.robot.Robot):
        while True:
            self.canvas.create_image(300,225,anchor="center",image=cozmoProcessImage(self.cozmoGetImage(robot),robot))
            self.canvas.configure(bg="lightblue")
            self.canvas.pack()
            time.sleep(CONFIG.refreshTime) 
